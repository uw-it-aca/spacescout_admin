...
class Spot1872(TestCase):
    def test_post(self):
        ...
        response = self.client.post("/api/v1/space/", '{ "name": "test_space_1872", "manager": "javerage" }', content_type="application/json")

        data = json.loads(response.content)

        url = "/api/v1/space/%s/" % data['id']
        response = self.client.put(url, '{"name":"ok-1872","manager":"javerage","editors":""}', content_type="application/json")
